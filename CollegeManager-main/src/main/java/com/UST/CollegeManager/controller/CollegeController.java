...
public class CollegeController {
    ...
    public ResponseEntity<Teacher> addFaculty(@PathVariable Long collegeId, @RequestBody Teacher faculty) {
        Teacher newFaculty = collegeService.addFaculty(collegeId, faculty);
        return ResponseEntity.ok(newFaculty);
    }

    @GetMapping("/{collegeName}/faculties")
    public ResponseEntity<List<Teacher>> getFacultiesByCollegeName(@PathVariable String collegeName) {
        List<Teacher> faculties = collegeService.getFacultiesByCollegeName(collegeName);
